<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Télécharger le classement</title>
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/styleClassementSecretaire.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;700&display=swap" rel="stylesheet">

</head>

<body>

    <header>

        <div class="logo">ProjetSelection</div>

        <ul class="navlinks">
            
            <li><a href="main.html">Accueil</a></li>
            <li><a href="interfaceGrille.html">Mes grilles</a></li>
            <li><a href="interfaceClassement.html">Mon classement</a></li>
            <li><a href="#">Contact</a></li>
            <li><a href="about.html">A propos</a></li>
            <li><a href="index.html">Déconnexion</a></li>
            <div class="dark-mode-switch">
                <input id="dark-mode" type="checkbox" onclick="document.documentElement.classList.toggle('dark-mode')"> 
                <label id="switch" for="dark-mode">White 
        </ul>


    </header>

    <div class="header2">
        Vous êtes connecté sur votre espace personnel en tant que Craig Federighi ( Secrétaire )
    </div>

  
    <div class="table">
        <table class="tableau">

            <b>Consulter le classement</b>
            <a href="">Cliquez ici pour télécharger le classement en fichier .csv</a>
            
            
            <thead>
                <tr>
                    
                    <th>Position</th>
                    <th>Nom</th>
                    <th>Prénom</th>
                    <th>Note</th>

                </tr>

            </thead>

            <tbody>
                <tr>
                    
                    <td>1</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>2</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>

                <tr>
                    
                    <td>3</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>

                <tr>
                    
                    <td>4</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>

                <tr>
                    
                    <td>5</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>6</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>7</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>8</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>9</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>10</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>11</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>12</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>13</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>14</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>
                <tr>
                    
                    <td>15</td>
                    <td>Williams</td>
                    <td>Jeff</td>
                    <td>20/20</td>
                    
                    
                </tr>



             
                
            </tbody>


        </table>

    </div>





    <footer>
        Egnom, Inc (C) - 01/04/1976

    </footer>






</body>

</html>